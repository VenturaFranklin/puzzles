...
spherical capsules that are carried to the top and allowed to bounce
...
the capsule bounces off the disc and soars away. You feel compelled
...
and one second elapses as the capsule passes from one disk to the one below it.
So, if you push the button at time=100, then the capsule reaches the
top disc at time=101, the second disc at time=102, the third disc at
time=103, and so on.
...
4 at time=0, by time=1 it has ticked one position forward.
...
it started at position 1, then continued to position 0, and finally
ended up at position 1 again. Because there's only a slot at position 0,
the capsule bounces away.

If, however, you wait until time=5 to push the button,
then when the capsule reaches each disc, the first disc will have ticked
forward 5+1 = 6 times (to position 0), and the second disc will have ticked
forward 5+2 = 7 times (also to position 0). In this case, the capsule would
...
if __name__ == "__main__":
    test_1()
#     input = [(17, 5), (19, 8), (7, 1), (13, 7), (5, 1), (3, 0)]
